...
file_name=$(printf "%s" "${files_result}" | grep -o '"file_name":"[^"]*\.img\.xz"' | head -1 | sed 's/.*"file_name":"//; s/"$//' || true)
...
curl -sk --fail --header "JOB-TOKEN: ${CI_JOB_TOKEN}" -o "${output_dir}/${file_name}" "${download_url}"
